...
function enterKey(e) {
    ...
    if (e.keyCode == 40 && git != commands.length) {
        git += 1;
        if (commands[git] === undefined) { textarea.value = ""; } 
        else { textarea.value = commands[git]; } 
        command.innerHTML = textarea.value;
    }
}
...
function router(cmd){
    switch (cmd.toLowerCase()) {
        ...
        case "education":
            printLines(education, "color2 margin", 80)
            break;
        ...
        case "email":
            addLine('Opening mailto:<a href="mailto:pvarsh@umich.edu">pvarsh@umich.edu</a>...', "color2", 80);
            openNewTab(email);
            break;
...
function printLines(name, style, time){ name.forEach((item, index) => { addLine(item, style, index*time); } ) }
